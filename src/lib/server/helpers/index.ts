...
const ENC_SALT = 'YEARBOOK_SALT'

export const getPackageVersion = (pkg: string) => {
	// Runtime, not `$env/static/private`: the built image must not carry the build
	// machine's NODE_ENV or working directory baked into it. See `$lib/server/env`.
...
export const cleanFileExtension = (ext: string): string => {
	return ext.replace(/^[.\s]+|[.\s]+$/g, "")
};
...
export const encrypt = async (input: string) => {
	...
	const header = 'YEARBOOK/' + initVector.toString('hex') + '/';
